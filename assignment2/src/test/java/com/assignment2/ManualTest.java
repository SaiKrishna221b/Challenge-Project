...
public class ManualTest {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            double actual = analyzer.calculateTotalRevenue();
            if (Math.abs(expected - actual) < 0.01) {
                System.out.println("[PASS] Calculate Total Revenue");
                passed++;
            } else {
                System.err.println("[FAIL] Calculate Total Revenue. Expected: " + expected + ", Got: " + actual);
                failed++;
            }
        } catch (Exception e) {
            System.err.println("[FAIL] Calculate Total Revenue threw exception: " + e.getMessage());
            failed++;
        }
        ...
        try {
            ...
            if (Math.abs(revenue.get("Electronics") - 1100.00) < 0.01 && 
                ...
                System.out.println("[PASS] Revenue By Category");
                passed++;
            } else {
                System.err.println("[FAIL] Revenue By Category. Expected {Electronics=1100, Clothing=200}, Got: " + revenue);
                failed++;
            }
        } catch (Exception e) {
            System.err.println("[FAIL] Revenue By Category threw exception: " + e.getMessage());
            failed++;
        }

        // Test 3: Top Selling Product
        try {
            ...
            if (top.isPresent() && top.get().getKey().equals("Mouse") && top.get().getValue() == 5) {
                System.out.println("[PASS] Top Selling Product");
                passed++;
            } else {
                System.err.println("[FAIL] Top Selling Product. Expected Mouse (5), Got: " + (top.isPresent() ? top.get().getKey() : "None"));
                failed++;
            }
        } catch (Exception e) {
            System.err.println("[FAIL] Top Selling Product threw exception: " + e.getMessage());
            failed++;
        }

        // Test 4: Sales Count by Region
        try {
            Map<String, Long> counts = analyzer.getSalesCountByRegion();
            if (counts.get("North") == 2 && counts.get("South") == 1 && counts.get("West") == 1) {
                System.out.println("[PASS] Sales Count By Region");
                passed++;
            } else {
                System.err.println("[FAIL] Sales Count By Region. Expected {North=2, ...}, Got: " + counts);
                failed++;
            }
        } catch (Exception e) {
            System.err.println("[FAIL] Sales Count By Region threw exception: " + e.getMessage());
            failed++;
        }
